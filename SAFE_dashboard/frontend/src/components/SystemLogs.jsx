...
export default function SystemLogs({
  ...
  // Memoize path statistics to prevent recalculations
  const pathStats = useMemo(() => {
    if (!paths || Object.keys(paths).length === 0) {
      return { total: 0, longest: 0, shortest: Infinity };
    }

    const pathLengths = Object.values(paths).map((path) => path.length);
    return {
  ...
  // Memoize formatted time to prevent unnecessary updates
  ...
  // Generate initial logs only when systemMode or paths change
  useEffect(() => {
    const newLogs = [
      {
        id: 1,
    ...
    // Add path calculation logs
    if (paths && Object.keys(paths).length > 0) {
      newLogs.push({
        id: Date.now() - 500,
        time: new Date().toLocaleTimeString(),
        message: `A* algorithm computed ${Object.keys(paths).length} evacuation paths`,
        type: "success",
      });

      // Add log for each source node (limit to avoid too many logs)
      Object.keys(paths)
        .slice(0, 5)
        .forEach((sourceNode, idx) => {
      ...
      // If there are more than 5 paths, add a summary
      if (Object.keys(paths).length > 5) {
        newLogs.push({
          id: Date.now() - 350,
          time: new Date().toLocaleTimeString(),
          message: `... and ${Object.keys(paths).length - 5} more paths`,
          type: "info",
        });
      }
    }
    ...
    setLogs(newLogs);
  }, [systemMode, paths]); // Only depend on systemMode and paths
  ...
  // Memoize path visualization to prevent unnecessary re-renders
  const renderPathVisualization = useCallback(
    (path, index) => {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">
              System Logs & Path Visualization
            </h3>
          </div>
        ...
        {paths && Object.keys(paths).length > 0 && (
          <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
            ...
            {Object.entries(paths).map(([sourceNode, path], index) => (
              <div key={sourceNode}>{renderPathVisualization(path, index)}</div>
            ))}
          </div>
        )}
          ...
          <div className="flex items-center gap-3">
            ...
            {paths && Object.keys(paths).length > 0 && (
              <span className="text-slate-500">
                {Object.keys(paths).length} active paths
              </span>
            )}
          </div>
        </div>
