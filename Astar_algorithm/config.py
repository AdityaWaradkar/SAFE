...
# Safety thresholds
T_SAFE = 40     # temperature threshold (°C)
F_CRITICAL = 60 # flame threshold (percentage)
SMOKE_CRITICAL = 70  # smoke threshold (percentage)
...
ROOM_NODES = (
    NODE_LOCATIONS["Room"] + NODE_LOCATIONS["Conference Room"]
)

# Primary safe destinations (exits and safe room)
PRIMARY_GOAL_NODES = (
    NODE_LOCATIONS["Safe Room"] + NODE_LOCATIONS["Exits"]
)

# Secondary safe destinations (rooms that can serve as temporary shelters)
SECONDARY_GOAL_NODES = (
    NODE_LOCATIONS["Room"] + NODE_LOCATIONS["Conference Room"]
)

# All nodes combined for reference
ALL_NODES = (
    NODE_LOCATIONS["Room"] + 
    NODE_LOCATIONS["Corridor"] + 
